        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex-1">
            ...
            <p className="mt-1 text-[color:var(--ob-muted)]">
              Every model available on {brandName}, with live latency and
              pricing.
            </p>
          </div>
          <SearchField
          ...
          {CATALOGUE.map((model) => (
            <Card key={model.id} className="flex flex-col gap-3">
              ...
              <dl className="mt-auto grid grid-cols-2 gap-3 border-t border-[color:var(--ob-border)] pt-3 text-[0.82rem]">
                <div>
                  <dt className="text-[color:var(--ob-muted)]">
                    Median latency
                  </dt>
        ...
        <Card padded={false}>
          <Table
            ...
            rows={ASSETS.map((asset) => [
              ...
              <span key="c" className="text-[color:var(--ob-muted)]">
                {asset.created}
              </span>,
              <Chip
                key="st"
                tone={asset.storage === "Archive" ? "neutral" : "brand"}
              >
                {asset.storage}
              </Chip>,
            ])}
      ...
      <div className="grid gap-6 px-6 py-8 sm:px-10">
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex-1">
            ...
            <p className="mt-1 text-[color:var(--ob-muted)]">
              Deployments scale to zero between requests. Cold start is the cost
              of that.
            </p>
          </div>
          <Btn tone="dark" size="sm">
        ...
        <Card padded={false}>
          <Table
            ...
            rows={DEPLOYMENTS.map((d) => [
              <span key="n" className="font-mono text-[0.86rem] font-medium">
                {d.name}
              </span>,
              <Chip
                key="s"
                tone={d.status === "Running" ? "success" : "neutral"}
              >
                {d.status}
              </Chip>,
              d.region,
      ...
      <Card>
        ...
        <LineChart
          ...
          series={[
            {
              id: "spend",
              points: [4, 6, 5, 9, 14, 11, 8, 12, 18, 16, 21, 19, 24, 22],
              color: "var(--ob-brand)",
              area: true,
            },
          ]}
      ...
      <Card padded={false}>
        ...
        <Table
          ...
          rows={ENDPOINTS.map((e) => [
            <span key="p" className="font-mono text-[0.86rem]">
              {e.path}
            </span>,
            <span key="c" className="tabular-nums">
              {e.calls.toLocaleString()}
            </span>,
            <span key="s" className="font-semibold tabular-nums">
              {e.spend}
            </span>,
          ])}
        />
